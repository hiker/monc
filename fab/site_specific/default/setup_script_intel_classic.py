...
def setup_script_intel_classic(build_config: BuildConfig,
    ...
    ifort.add_flags(["-g", "-traceback", "-fp-model",  "precise"], "base")
    ifort.add_flags(ContainFlags("/model_core/", "-recursive"), "base")
    ifort.add_flags(ContainFlags("/io/", "-recursive"), "base")

    icc.add_flags(["-g", "-traceback", "-std=gnu99"], "base")
    ...
    # Debug
    # =====
    ifort.add_flags(["-O2", "-check bounds,uninit", "-no-vec"], "debug")

    # Disable all checks for the files in psrc:
    for pattern in psrc:
        ifort.add_flags(ContainFlags(pattern, ["-nocheck"]), "safe")
    ...
    ifort.add_flags(["-O2", "-no-vec"], "high")
